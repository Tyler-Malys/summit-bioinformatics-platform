...
if (!nzchar(DE_CSV) || !nzchar(GMT_FILE) || !nzchar(GSEA_ROOT)) {
  stop(
    "Missing required args. Provide --de_csv=... --gmt_file=... --gsea_root=...",
    call. = FALSE
  )
}

# Where outputs go (derived)
OUT_BASE  <- file.path(GSEA_ROOT, RUN_ID)
OUT_RANKS <- file.path(GSEA_ROOT, "ranks")
OUT_LOGS  <- file.path(GSEA_ROOT, "logs")
OUT_MSIGDB <- file.path(GSEA_ROOT, "msigdb")
...
dir.create(OUT_MSIGDB, recursive = TRUE, showWarnings = FALSE)

if (!file.exists(DE_CSV)) {
  stop(sprintf("DE file not found: %s", DE_CSV), call. = FALSE)
}
if (!file.exists(GMT_FILE)) {
  stop(sprintf("GMT file not found: %s", GMT_FILE), call. = FALSE)
}
if (!dir.exists(GSEA_ROOT)) {
  stop(sprintf("GSEA root directory not found: %s", GSEA_ROOT), call. = FALSE)
}
...
message("GSEA_ROOT: ", normalizePath(GSEA_ROOT))
