...
print_usage <- function() {
  cat("Usage:\n")
  cat("  Rscript analysis/01_build_analysis_object.R \\\n")
  cat("    --counts-path <gene_counts.csv> \\\n")
  cat("    --out-dds <dds.rds> \\\n")
  cat("    --out-vsd <vsd.rds>\n")
}
...
counts_path <- get_flag_value("--counts-path")
out_dds     <- get_flag_value("--out-dds")
out_vsd     <- get_flag_value("--out-vsd")

if (is.null(counts_path) || is.null(out_dds) || is.null(out_vsd)) {
...
if (!file.exists(counts_path)) {
  stop(paste0("ERROR: counts file not found: ", counts_path), call. = FALSE)
}
...
cat("Output DDS:", out_dds, "\n")
cat("Output VSD:", out_vsd, "\n")
...
counts <- counts[rownames(counts) != "" & !is.na(rownames(counts)), , drop = FALSE]
...
# 9. Save Objects
# ===============================
dir.create(dirname(out_dds), showWarnings = FALSE, recursive = TRUE)
dir.create(dirname(out_vsd), showWarnings = FALSE, recursive = TRUE)
