...
print_usage <- function() {
  cat("Usage:\n")
  cat("  Rscript analysis/03_differential_expression.R \\\n")
  cat("    --dds-path <dds.rds> \\\n")
...
dds_path <- get_flag_value("--dds-path")
out_dir  <- get_flag_value("--out-dir")

if (is.null(dds_path) || is.null(out_dir)) {
...
if (!file.exists(dds_path)) {
  stop(paste0("ERROR: DDS file not found: ", dds_path), call. = FALSE)
}

dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

cat("Loading DDS object from:", dds_path, "\n")
