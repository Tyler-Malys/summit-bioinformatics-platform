...
write_stage_session_info("42_differential_cellranger")

infile <- "analysis/objects/pbmc1k_cellranger_annotated_sce.rds"
out_file <- "analysis/markers/pbmc1k_cellranger_Bcells_vs_Tcells_DE.tsv"
...
group <- ifelse(
  ...
  ifelse(labels %in% c("T cells", "IL7R+ T cells", "naive / resting T cells"), "T_cells", NA)
)
...
res <- res[, c("feature_id", "gene_id", "gene_name", setdiff(colnames(res), c("feature_id", "gene_id", "gene_name")))]
